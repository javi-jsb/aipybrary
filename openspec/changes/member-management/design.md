...
**Bundle the `database-seeding` delta into this change.** Seeding example members ships here, not as a separate change/issue. Precedent: both archived feature changes that introduced entities (`2026-05-15-books-crud`, `2026-05-18-book-list-enhancements`) carried their own `database-seeding` delta — seeding is part of delivering an entity for dev/manual testing, not an independent capability. Idempotency becomes **per-entity**: `seed.py` checks books and members independently, so adding member-seeding to an already-book-seeded DB still seeds members. The existing book-seeding requirement and its idempotency scenario remain unchanged (still true under per-entity logic), so the delta is purely `ADDED` requirements — no `MODIFIED`. Alternative considered: a separate `seed-members` change/issue — rejected as inconsistent with the established books precedent and unnecessary ceremony for a dev script.
