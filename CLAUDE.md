...
The Makefile wraps the common backend commands — run `make help` (or read the `Makefile`) for the target list, and prefer it for day-to-day work. It is a convenience, not a hard rule: invoking `uv run ...` directly is fine when it keeps a component self-contained (e.g. the E2E harness spawns the backend from its Playwright config/fixtures rather than shelling out to `make`). Non-obvious point worth knowing:
...
#### Role-based authorization (the security boundary)

Authentication proves *identity*; authorization (in `app/users/infrastructure/authz.py`) enforces *what each role may do* server-side — it **is** the security boundary, and the frontend's `roles.ts` only mirrors it for UX. `get_current_user` returning `401` (missing/invalid token) is kept distinct from `403` (valid token, wrong role/ownership). Primitives:

- `require_role(*roles)` — dependency factory; `staff_only = require_role(admin, staff)` is the shared instance for catalog/member/loan **writes**. Reads stay on the plain auth gate, open to any authenticated user.
- `require_self_or_staff` — ownership gate for `GET /members/{id}`: `admin`/`staff` always; a `member` only for their own record (resolved via `MemberRepository.get_by_user_id`).
- Loans are **scoped, not just gated**: `GET /loans` forces `member_id` to the caller's own member for a `member` (overriding any filter), and `GET /loans/{id}` returns `403` for another member's loan. Borrow/return/undo/cancel are `staff_only`.

The matrix: manage books/copies/members + borrow/return → `admin`/`staff`; a `member` reads the catalog, their own profile, and their own loans only. `tests/conftest.py` provides `admin_client`/`member_client` (role-only) and `member_account` (a member linked to a persisted `Member`, for ownership/scoping); `tests/test_authorization.py` is the per-endpoint matrix.
...
**Role-aware UI.** `useCurrentUser` (`GET /auth/me`) exposes the role; `src/auth/roles.ts` centralizes the role→action gating (e.g. `canManageBooks` → `admin`/`staff` see create/edit/delete; `member` is read-only). For members, `canManageMembers` (`admin`/`staff`) gates create/edit, while `canDeleteMembers` (`admin` only) gates the destructive delete — the matrix is pinned per entity slice, so it can diverge between entities. For book copies, `canManageCopies` (`admin`/`staff`) gates the add/remove controls in the copies view (`/books/:id/copies`), which any authenticated user may open read-only. For loans, `canManageLoans` (`admin`/`staff`) gates the borrow form (`/loans/new`) and the per-loan return action in the loans view (`/loans`), which any authenticated user may open read-only. `canViewMembers` (`admin`/`staff`) gates the members list, its routes, and its nav entry — a `member` has no members nav and is scoped server-side to their own record, so the loans view skips the `/members` fetch (it would `403`) and labels each row with the caller's own email. The backend enforces role authorization (see "Role-based authorization") and **is** the security boundary; the frontend gating merely mirrors it and is not itself a boundary. Two distinct failure paths back this up: a `403` is handled gracefully (the `RequireRole` route guard renders `ForbiddenScreen` for a deep link the nav hides; `apiErrorToFormMessage` surfaces a friendly "not allowed" for an in-view `403`), kept separate from a `401`/session-expired path — `apiClient`'s `setUnauthorizedHandler` (wired by `AuthProvider`) drops the session on any `401`, so `ProtectedLayout` redirects to login.
...
A separate **end-to-end** layer (the `e2e-testing` capability spec) drives the real SPA in a real Chromium browser against the real API and a dedicated database — the gap Vitest's stubbed `fetch` can't cover. Specs live in `frontend/e2e/` as `*.spec.ts` and use `@playwright/test` (pinned, Chromium-only); config is `frontend/playwright.config.ts`. Run with `pnpm test:e2e` (from `/frontend`) or `make e2e-frontend`. **Local execution only — no CI integration.** Prerequisites: Postgres up (`make db-up`) and the one-time `make e2e-setup` (frontend deps + Chromium, not committed). Vitest excludes `e2e/` (both use `*.spec.ts`).

The harness is **self-contained**: `e2e/config.ts` holds the shared wiring (dedicated ports/db) and Playwright spawns the backend directly via `uv` (not the Makefile). `webServer` boots both tiers on **dedicated ports** so the E2E stack never collides with a running `make dev`/`dev-frontend` (8077/5173): the FastAPI API on **:8078** against the **`aipybrary_e2e`** database, and the Vite SPA on **:5273** pointed at it. `globalSetup` provisions the DB once (create + Alembic migrate + seed); `e2e/fixtures.ts` truncates + re-seeds before every test and exposes `loginAs(role)`, which authenticates against the real API and injects the token so the next navigation lands authenticated. The deterministic seed and the destructive-op guard (refuses any DB whose name isn't `*e2e`) live in `scripts/e2e_db.py` (invoked by the harness with `POSTGRES_DB` set). Seeded logins (`admin`/`staff`/`member`@aipybrary.dev, password `pass`). Per-area flows are covered by one spec per area in `frontend/e2e/` (auth, books, members, copies, loans, role-aware visibility); they reuse seeded fixtures (e.g. a "Maxed Member" at the active-loan limit drives the loan-limit error path). When a flow needs specific data, extend the seed in `scripts/e2e_db.py`.
