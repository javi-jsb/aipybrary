...
A separate **end-to-end** layer (`e2e-playwright-tests` OpenSpec change, umbrella #93) drives the real SPA in a real Chromium browser against the real API and a dedicated database — the gap Vitest's stubbed `fetch` can't cover. Specs live in `frontend/e2e/` as `*.spec.ts` and use `@playwright/test` (pinned, Chromium-only); config is `frontend/playwright.config.ts`. Run with `pnpm test:e2e` (from `/frontend`) or `make e2e-frontend`. **Local execution only — no CI integration.** Prerequisites: Postgres up (`make db-up`) and Chromium installed once (`pnpm exec playwright install chromium`, not committed). Vitest excludes `e2e/` (both use `*.spec.ts`).

Playwright's `webServer` boots both tiers on **dedicated ports** so the E2E stack never collides with a running `make dev`/`dev-frontend` (8077/5173): the API on **:8078** against the **`aipybrary_e2e`** database (`make e2e-api`), and the Vite SPA on **:5273** pointed at it. `globalSetup` provisions the DB once (create + Alembic migrate + seed via `make db-e2e-provision`); `e2e/fixtures.ts` truncates + re-seeds before every test (`make db-e2e-reset`) and exposes `loginAs(role)`, which authenticates against the real API and injects the token so the next navigation lands authenticated. The deterministic seed and the destructive-op guard (refuses any DB whose name isn't `*e2e`) live in `scripts/e2e_db.py`. Seeded logins (`admin`/`staff`/`member`@aipybrary.dev, password `pass`). Per-area flows are covered by one spec per area in `frontend/e2e/` (auth, books, members, copies, loans, role-aware visibility); they reuse seeded fixtures (e.g. a "Maxed Member" at the active-loan limit drives the loan-limit error path). When a flow needs specific data, extend the seed in `scripts/e2e_db.py`.
