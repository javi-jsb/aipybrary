...
Playwright's `webServer` boots both tiers on **dedicated ports** so the E2E stack never collides with a running `make dev`/`dev-frontend` (8077/5173): the API on **:8078** against the **`aipybrary_e2e`** database (`make e2e-api`), and the Vite SPA on **:5273** pointed at it. `globalSetup` provisions the DB once (create + Alembic migrate + seed via `make db-e2e-provision`); `e2e/fixtures.ts` truncates + re-seeds before every test (`make db-e2e-reset`) and exposes `loginAs(role)`, which authenticates against the real API and injects the token so the next navigation lands authenticated. The deterministic seed and the destructive-op guard (refuses any DB whose name isn't `*e2e`) live in `scripts/e2e_db.py`. Seeded logins (`admin`/`staff`/`member`@aipybrary.dev, password `pass`). The per-area UI flows arrive in #98.
