...
async def get_loan(loan_id: uuid.UUID, service: ServiceDep) -> LoanPublic:
...
async def borrow(data: LoanCreate, service: ServiceDep) -> LoanPublic:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="Member is suspended"
        ) from None
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Book copy is already on loan"
        ) from None
...
async def return_loan(loan_id: uuid.UUID, service: ServiceDep) -> LoanPublic:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Loan is already returned"
        ) from None
    if loan is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Loan not found")
    return LoanPublic.model_validate(loan)
...
@router.delete("/{loan_id}/return", response_model=LoanPublic)
async def undo_return(loan_id: uuid.UUID, service: ServiceDep) -> LoanPublic:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Loan is not returned"
        ) from None
    if loan is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Loan not found")
    return LoanPublic.model_validate(loan)
...
async def cancel_loan(loan_id: uuid.UUID, service: ServiceDep) -> None:
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Cannot cancel a returned loan"
        ) from None
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Loan not found")
